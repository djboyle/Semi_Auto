...
import uuid
import logging
from Semi_Auto.auth import AccessTokenAuth, APIKeyAuthWithExpires
from Semi_Auto.utils import constants, errors
from Semi_Auto.ws.ws_thread import BitMEXWebsocket
...
class BitMEX(object):
    ...
    @authentication_required
    def buy(self, type, quantity, price):
        """Place a buy order.

        Returns order object. ID: orderID
        """
        return self.place_order(type, quantity, price)

    @authentication_required
    def sell(self, type, quantity, price):
        """Place a sell order.
    ...
    @authentication_required
    def get_Candles(self, binSize, Symbol, Fee):
        api = "quote/bucketed"
        postdict = {
            'binSize': '1h',
            'symbol': 'XBt',
    ...
    @authentication_required
    def place_order(self, type, quantity, price):
        """Place an order."""
        if price < 0:
    ...
    def close_open_position(self):
        """close all positions"""
        return self._curl_bitmex(api='order/closePosition', postdict={'symbol': self.symbol}, verb="POST")
